...
/**
 * Dollars (already-in-dollars) → locale-formatted "$X,XXX". Defaults to
 * whole dollars (no decimals). Mirrors `safePriceCentsLocale` but skips
 * the cents-to-dollars conversion. Returns the placeholder for any
 * non-finite / non-positive input.
 */
export function safePriceLocale(
  value: unknown,
  opts: SafePriceLocaleOpts = {},
): string {
  const placeholder = opts.placeholder ?? DEFAULT_PLACEHOLDER;
