...
- End-to-end verification (real send → real inbox → real events) is a manual ops task. See `docs/runbooks/sendgrid-event-webhook-verification.md` for the operator checklist.
...
- Four patterns are forbidden outside `safePrice.ts`: raw `` `$${...}` `` template literal interpolation; JSX text ending in `$` immediately before a `{...}` expression; string concatenation that prefixes `$` (e.g. `"$" + value`); and array `join` of a `$` literal. Enforced two ways: (1) `tests/lib/noRawPriceTemplates.test.ts` (AST scan via `@typescript-eslint/typescript-estree`) for CI, and (2) the local ESLint rule `safe-price/no-raw-price-format` (defined in `eslint-rules/no-raw-price-format.cjs`, wired into `eslint.config.js` for `client/src/**`, `server/**`, and `shared/**`) so editors and the `safe-price-eslint` validation workflow flag the same patterns at edit/commit time. The rule keeps `client/src/lib/safePrice.ts` and `shared/safePrice.ts` allowlisted.
- Full guide and helper picker: `docs/safe-price-rule.md`. Run the Jest check with `npx jest --selectProjects lib --testPathPatterns noRawPriceTemplates --no-cache`, and the standalone ESLint check with `npx eslint --config eslint.safeprice.config.js 'client/src/**/*.{ts,tsx}' 'server/**/*.{ts,tsx}' 'shared/**/*.{ts,tsx}' 'tests/**/*.{ts,tsx}' 'e2e/**/*.{ts,tsx}'` (also registered as the `safe-price-eslint` validation workflow). Always invoke ESLint from the repo root so the allowlist's relative paths match.
- Push-time enforcement (Task #172): `scripts/safe-price-checks.sh` runs both gates back-to-back and is wired into two places. (1) A versioned pre-push git hook at `scripts/git-hooks/pre-push` — installed by `scripts/install-git-hooks.sh` (which sets `git config core.hooksPath scripts/git-hooks`) and re-installed automatically by `scripts/post-merge.sh` so every contributor picks it up after the next merge. (2) A GitHub Actions workflow at `.github/workflows/safe-price.yml` that runs on every `push` and `pull_request`, so a developer who skipped the editor warnings AND the hook still gets blocked at the PR level. Emergency bypass for the hook only: `SKIP_SAFE_PRICE=1 git push` or `git push --no-verify` (CI still runs and will block the PR).
- Test/e2e scope expansion (Task #178): the standalone ESLint config (`eslint.safeprice.config.js`) and the Jest scanner (`tests/lib/noRawPriceTemplates.test.ts`) also walk `tests/**/*.{ts,tsx}` and `e2e/**/*.{ts,tsx}`, so a regression that lands in a test fixture or seeded data is caught by the same gates. The unavoidable plan-price report fixture `tests/monetization/stripe-tests.ts` is allowlisted in both `eslint-rules/no-raw-price-format.cjs` and the Jest scanner's `ALLOWLIST` (the labels are console/JSON test output only, never user-facing). The `scripts/safe-price-checks.sh` ESLint command line and the `safe-price-eslint` validation workflow are kept in sync with the new scope.

## Booking-link slug backfill (Task #189)
- One-shot script: `tsx server/scripts/backfillBookingSlugs.ts` (add `--dry-run` to preview). Sweeps every row where `users.public_profile_slug IS NULL`, derives a base slug with `generateBookingSlug` (mirroring `DbStorage.computeBaseSlugForNewUser` — `businessName` → `name` → `username` → email-local-part, falling back to `user-<id-prefix>` when nothing is usable or the derived slug is the generic `pro` placeholder), and writes through `writeUserSlugWithRetry` so collisions auto-suffix (`-2`, `-3`, …) under the partial unique index `users_public_profile_slug_unique_idx`.
- Idempotent: the SELECT only matches NULL rows and the UPDATE is gated on `public_profile_slug IS NULL`, so re-runs (and concurrent signups racing the script) never clobber an existing slug.
- Verification query (run after the script — should return 0): `SELECT COUNT(*) FROM users WHERE public_profile_slug IS NULL;`. The script prints this count at the end of every non-dry-run.
