...
function generateConnectPayload(eventType: string, dataObject: Record<string, any>, eventId?: string) {
  return JSON.stringify({
    id: eventId || `evt_connect_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
    object: 'event',
...
async function sendConnectWebhook(payload: string, headers: Record<string, string> = {}) {
...
// When STRIPE_CONNECT_WEBHOOK_SECRET is not set, the connect webhook handler
// falls back to using req.body (parsed JSON) directly without signature
// verification. This allows end-to-end event processing tests.
//
// When the secret IS set, signature verification hits the same raw-body issue
// as the platform webhook (see stripe.platform.webhook.test.ts TODO), so
// those tests are skipped until the underlying issue is fixed.
...
describe('Stripe Connect Webhook (/api/stripe/connect/webhook)', () => {
  ...
  describeWithSecret('Signature enforcement (secret configured)', () => {
    ...
    // TODO: Valid signature tests skipped — see platform webhook TODO.
    // constructEvent receives parsed req.body (JS object), not raw Buffer.
  });

  describeNoSecret('Event processing (no secret — fallback to parsed body)', () => {
    ...
    it('processes customer.subscription.created', async () => {
      const payload = generateConnectPayload('customer.subscription.created', {
